...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!correo || !contrasena) { setError("Escribe tu correo y contraseña"); return }
    setCargando(true)
    setError("")
  ...
  return (
    ...
    >
      <div className="w-full max-w-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-4" noValidate>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  id="contrasena" type={mostrarPass ? "text" : "password"} value={contrasena}
            ...
            <button type="submit" disabled={cargando}
              className="w-full flex items-center justify-center gap-2 bg-brand hover:bg-amber-500 text-white font-semibold py-3.5 rounded-xl transition-colors disabled:opacity-60 disabled:cursor-not-allowed text-base mt-2">
              {cargando ? (
        ...
        <p className="text-center text-sm text-gray-600 mt-6">¿Problemas para entrar? Contacta al administrador.</p>
      </div>
    </div>
  )
}
